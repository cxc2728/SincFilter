...
void OnFrequencyDomain(char filterType, char filter_kz, char imageFilename[], int rcxres, int rcyres, double m_Real, double m_Imaginary, int m_angles);
...
int main(int argc, char* argv[]) {
	...
	if (argc < 15) {
		...
		std::cout << "Example parameters for LPF: l 2.5 5 0.0287 0.287 b 0.5 k 0.5 0.5 5" << endl;
		std::cout << "Example parameters for HPF: h 2.5 5 0.0287 0.287 b 0.5 k 0.5 0.5 5" << endl;
		std::cout << endl;
		exit(0);
	}
		...
		if ((savedata = fopen(outputFile, "w")) == NULL)
		{
			...
			OnFrequencyDomain(filterType, filter_kz, imageFileName, n1, n2, m_Real, m_Imaginary, m_angles);
...
void OnFrequencyDomain(char filterType, char filter_kz, char imageFilename[], int rcxres, int rcyres, double m_Real, double m_Imaginary, int m_angles)
{
			...
			for (j = 0; j < NofXpixels; j++)
			{
				...
				{ ///calculate Radon-Space data 
					for (int p = 0; p < NofXpixels - 1; p++)
					{
						...
						double m_TheSamplingRate = 1.0;
						double m_Bandwidth = 0.5;

						double dty = ((double)((double)s - NofYpixels / 2) * m_TheSamplingRate);
						double dtx = ((double)((double)p - NofXpixels / 2) * m_TheSamplingRate);

						double dy_y = (double)pi * (2.0 * m_Bandwidth * y_z - dty);
						double dx_x = (double)pi * (2.0 * m_Bandwidth * x_z - dtx);
	...
	//// Procedure to scale the filter (begin)
	 // scale tf (begin) 
	double max = *(RadonSincFilter);
	double min = *(RadonSincFilter);
